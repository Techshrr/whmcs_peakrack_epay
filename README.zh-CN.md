...
后台表单会根据当前 `签名方式` 只显示需要填写的密钥字段。切换模式时，被隐藏的旧密钥不会被清空；保存时会阻止当前模式缺少必填密钥的配置。
...
- 增加 CSS 优先的 `签名方式` 字段隐藏规则，使用 `:has()`，解决 WHMCS 通过动态插入网关配置但不执行内联脚本时，非当前模式密钥行不隐藏的问题。
- 增加按行顺序兜底的隐藏规则，兼容 WHMCS 把网关字段 name 渲染成其他格式的情况。
- 保留 JavaScript 切换逻辑，作为允许内联脚本执行环境下的额外兜底。

### 2.0.2

- 加强后台 `签名方式` 字段切换逻辑，兼容 WHMCS 把网关设置字段名渲染成不同大小写或不同表格结构的情况。
- 切换脚本现在会同时按设置字段名、可见行标题、V1/V2 下拉选项文字查找字段，并在页面加载后延迟重试。

### 2.0.1

- 增加按 `签名方式` 自动显示/隐藏密钥字段的后台交互。
- `V1 / MD5` 只显示并要求填写 `商户密钥 / KEY`。
- `V2 / RSA` 只显示并要求填写 `商户私钥 / PRIVATE KEY` 和 `平台公钥`。
- 切换模式时隐藏字段保留已保存值，不会被自动清空。
