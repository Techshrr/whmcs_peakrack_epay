...
本版本为后台 `签名方式` 选择器增加 CSS 优先的字段隐藏规则。WHMCS 有时会动态插入网关设置但不执行内联脚本；新的 CSS 规则仍会在 `V1 / MD5` 模式隐藏 RSA 密钥行，在 `V2 / RSA` 模式隐藏 MD5 商户密钥行。另加按行顺序兜底的隐藏规则，兼容网关字段 name 被渲染成其他格式的安装环境。
